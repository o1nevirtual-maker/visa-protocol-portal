...
exports.buildFullTypeDefinition = buildFullTypeDefinition;
exports.buildFunctionSelector = buildFunctionSelector;
exports.overloadArities = overloadArities;
exports.resolveFunctionFragment = resolveFunctionFragment;
exports.isReadOnlyFunctionFragment = isReadOnlyFunctionFragment;
...
function encodeFunctionData(funcABI, args = []) {
    ...
    const selector = fragments_js_1.FunctionFragment.from(funcABI).selector.replace(/^0x/, '');
...
function canonicalizeIntType(type) {
    return type.replace(/^(u?int)(\[|$)/, '$1256$2');
}
function buildFullTypeDefinition(typeDef) {
    if (typeDef && typeDef.type.indexOf('tuple') === 0 && typeDef.components) {
        const innerTypes = typeDef.components.map((innerType) => {
            return buildFullTypeDefinition(innerType);
        });
...
function buildFunctionSelector(fragment) {
...
function functionsNamed(abi, functionName) {
    return abi.filter((item) => item.type === 'function' && 'name' in item && item.name === functionName);
}
/**
 ...
 */
function isArgOfType(arg, input) {
    const type = input.type;
    if (type === 'bool')
    ...
    if (/^bytes\d*$/.test(type))
        return typeof arg === 'string' || arg instanceof Uint8Array;
    ...
    if (type.endsWith(']')) {
        ...
        if (!match)
            return true; // malformed suffix → stay lenient
        const [, elementType, fixedLength] = match;
        if (fixedLength !== '' && arg.length !== Number(fixedLength))
            return false;
        const element = { ...input, type: elementType };
    ...
    if (type.indexOf('tuple') === 0)
        return isTupleArgOfShape(arg, input.components);
    return true; // unknown type → accept
}
/** Whether `arg` (object or positional array) matches a tuple's component shape. */
function isTupleArgOfShape(arg, components) {
...
function overloadArities(abi, functionName) {
 ...
 */
function resolveFunctionFragment(abi, functionName, args) {
    // Explicit disambiguation by full signature, e.g. "transfer(address,uint256)".
    if (functionName.indexOf('(') !== -1) {
        const allFunctions = abi.filter((item) => item.type === 'function' && 'name' in item);
    ...
    if (byArity.length === 0) {
        ...
        throw new Error(`Contract function "${functionName}" expects ${expected} argument(s) but received ${argList.length}.`);
    }
    // Same arity — disambiguate by runtime argument types.
    const byType = byArity.filter((fragment) => (fragment.inputs ?? []).every((input, index) => isArgOfType(argList[index], input)));
    if (byType.length === 1)
        return byType[0];
    const signature = buildFunctionSelector(byArity[0]);
    throw new Error(`Ambiguous overloaded function "${functionName}" for the given arguments; ` +
        `pass the full signature (e.g. "${signature}") as functionName to disambiguate.`);
}
function isReadOnlyFunctionFragment(fragment) {
